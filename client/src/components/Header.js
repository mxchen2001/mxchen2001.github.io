...
const headerElements = [
  ...
  {
    name: 'Projects',
    description: 'My completed and work-in-progress side projects.',
    href: '#Projects',
    icon: BookmarkAltIcon,
  },
  {
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6">
            <div className="flex justify-between items-center border-b-2 border-gray-100 py-6 md:justify-start md:space-x-10">
              <div className="lg:w-0 lg:flex-1">
                <div className="flex justify-start -mr-2 -my-2 md:hidden">
                  <span className="text-2xl wave">👋</span>
                  <a href="#" className="pl-5 text-2xl font-medium text-gray-500 hover:text-gray-900">Home</a>
                </div>
              </div>
                        ...
                        <Popover.Panel static className="absolute z-10 -ml-4 mt-3 transform px-2 w-screen max-w-md sm:px-0 lg:ml-0 lg:left-1/2 lg:-translate-x-1/2">
                          <div className="rounded-lg shadow-lg ring-1 ring-black ring-opacity-5 overflow-hidden">
                            <div className="relative grid gap-6 bg-white px-5 py-6 sm:gap-8 sm:p-8">
                              <a className="text-center font-medium text-gray-500">Academics</a>
                              {undergradTa.map((item) => (
                                <a href={item.href} key={item.id} className="-m-3 p-3 flex items-center rounded-md hover:bg-gray-50">
                                  <item.icon className="flex-shrink-0 h-6 w-6 text-indigo-600" aria-hidden="true" />
                                  <span className="ml-3 text-base font-medium text-gray-900">{item.name}</span>
                                </a>
                              ))}
                            </div>
                            <div className="relative grid bg-gray-50 gap-6 bg-white px-5 py-6 sm:gap-8 sm:p-8">
                              <a className="text-center font-medium text-gray-700">Tools</a>
                              {resourceTools.map((item) => (
                                <a href={item.href} key={item.id} className="-m-3 p-3 flex items-center rounded-md hover:bg-gray-100">
                                  <item.icon className="flex-shrink-0 h-6 w-6 text-indigo-600" aria-hidden="true" />
                                  <span className="ml-3 text-base font-medium text-gray-900">{item.name}</span>
                                </a>
                              ))}
                ...
                <div className="pt-5 pb-6 px-5">
                  ...
                  <div className="mt-6">
                    <nav className="grid gap-y-8">
                      <a className="font-medium text-gray-500">Academics</a>
                      {undergradTa.map((item) => (
                        <a key={item.id} className="-m-3 p-3 flex items-center rounded-md hover:bg-gray-50">
                          <item.icon className="flex-shrink-0 h-6 w-6 text-indigo-600" aria-hidden="true" />
                          <span className="ml-3 text-base font-medium text-gray-900">{item.name}</span>
                        </a>
                      ))}
